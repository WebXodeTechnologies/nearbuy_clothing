...
import { HardDrive, PlusCircle, Search } from "lucide-react";
  ...
  const handleAllocateStorage = async (e) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/vendors/${selectedVendor._id}/storage`,
        {
          method: "PATCH",
      ...
      if (data.success) {
        toast.success(
          `Successfully added ${extraGB}GB to ${selectedVendor.storeName}`,
        );
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black font-heading">
              Cloud Storage & Quota Server
            </h1>
          </div>
          <p className="text-xs text-slate-400">
            Manage UploadThing server pools and allocate extra storage quotas to
            individual vendor IDs.
          </p>
        </div>
