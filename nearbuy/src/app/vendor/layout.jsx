    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center text-slate-300">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 space-y-5 shadow-2xl">
          <div className="h-14 w-14 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-2xl flex items-center justify-center mx-auto">
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-slate-400 leading-relaxed font-medium">
              You must be logged in as a registered Merchant Vendor to access
              the store dashboard.
            </p>
          </div>
          <Button
            onClick={handleBackToLogin}
            className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl py-3 shadow-lg shadow-blue-600/30 transition-all cursor-pointer"
          >
            Sign In as Vendor
          </Button>
        </div>
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-body text-slate-900">
      ...
      <div className="hidden lg:block shrink-0">
        <Sidebar type="vendor" isMobile={false} />
      </div>
      ...
      {isMobileSidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          ...
          />
          <div className="relative z-10 w-72 sm:w-80 h-full bg-slate-950 flex flex-col shadow-2xl overflow-y-auto">
            <button
              onClick={() => setIsMobileSidebarOpen(false)}
              className="absolute top-4 right-4 p-2.5 rounded-xl bg-slate-900 text-slate-400 hover:text-white border border-slate-800 z-30 cursor-pointer"
              aria-label="Close menu"
            >
              <X className="w-5 h-5" />
            </button>
            <div className="w-full h-full flex flex-col">
              <Sidebar type="vendor" isMobile={true} />
            </div>
          </div>
        </div>
      )}
        ...
        <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 py-3.5 flex items-center justify-between shadow-xs">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <Link
              ...
              target="_blank"
              className="hidden md:flex items-center gap-1.5 text-xs font-bold text-slate-700 hover:text-blue-600 bg-slate-100 hover:bg-blue-50 px-3.5 py-2 rounded-xl border border-slate-200 transition-colors"
            >
            ...
            <Link
              ...
              className="relative p-2 rounded-xl bg-slate-100 hover:bg-slate-200/80 text-slate-600 hover:text-blue-600 transition-colors border border-slate-200"
              title="Notifications"
            >
            ...
            <div className="flex items-center gap-2.5 pl-2 border-l border-slate-200">
              <div className="h-9 w-9 rounded-xl bg-linear-to-tr from-blue-600 to-sky-400 p-0.5 shadow-xs">
                <div className="w-full h-full rounded-[10px] bg-white flex items-center justify-center font-black text-blue-700 text-xs uppercase overflow-hidden">
                  {user?.image ? (
                    <Image
                      src={user.image}
                      alt={user.name}
              ...
              <div className="hidden lg:flex flex-col">
                ...
                <span className="text-[10px] font-bold text-emerald-600 flex items-center gap-0.5">
                  {user?.planName || "Pro Gold"}{" "}
                  <Sparkles className="w-2.5 h-2.5 fill-emerald-500 text-emerald-500" />
                </span>
              </div>
            </div>
          </div>
