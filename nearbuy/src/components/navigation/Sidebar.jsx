...
export default function Sidebar({ type = "vendor" }) {
  ...
  // Define sidebar menu options based on Admin vs Vendor type
  const vendorMenu = [
    {
  ...
  return (
    <aside className="w-64 bg-gray-900 border-r border-gray-800 flex flex-col justify-between shrink-0 h-screen sticky top-0 text-gray-300">
      {/* Brand Header */}
      <div>
        <div className="h-16 flex items-center px-6 border-b border-gray-800 bg-gray-950 gap-2.5">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-gray-400 font-medium">
              {type === "admin" ? "Administrator Portal" : "Merchant Account"}
            </span>
          </div>
        </div>
          ...
          {currentMenu.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge && (
                  <Badge variant={isActive ? "blue" : "gray"} pill className="text-[9px] px-1.5 py-0">
                    {item.badge}
                  </Badge>
                )}
              </Link>
      ...
      <div className="p-4 border-t border-gray-800 bg-gray-950">
        ...
        <div className="flex gap-2">
          <Link
            href="/"
            className="flex-1 text-center py-1.5 rounded-md text-xs font-medium text-gray-400 hover:text-white hover:bg-gray-800 transition-colors border border-gray-800"
          >
            Visit Site
          </Link>
          <button
            onClick={handleLogout}
            className="flex-1 text-center py-1.5 rounded-md text-xs font-medium text-red-400 hover:text-red-300 hover:bg-red-950/20 border border-red-900/35 transition-colors cursor-pointer"
          >
            Sign Out
          </button>
        </div>
