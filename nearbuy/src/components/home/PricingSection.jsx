...
function PricingCard({ plan, index }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -8 }}
      className={`relative rounded-3xl p-8 flex flex-col justify-between transition-all duration-300 overflow-hidden cursor-default select-none border min-h-135 ${
        popular
    ...
    >
      ...
      <div className="relative z-10 space-y-6">
        <div>
          <h3 className="text-xl font-bold tracking-tight text-slate-950 font-heading">
            {name}
          </h3>
          <p className="text-sm text-slate-500 mt-2 leading-relaxed font-body min-h-10">
            {description}
          </p>
        </div>
        ...
        <div className="flex items-baseline gap-1 mt-4">
          ...
          <span className="text-sm text-slate-500 font-semibold font-body">
            /{period}
          </span>
        </div>
        ...
        <div className="border-t border-slate-100 pt-6">
          ...
          <ul className="space-y-3 font-body">
            {features.map((feature, idx) => (
              ...
              >
                <svg
                  className={`w-4 h-4 mt-0.5 shrink-0 ${popular ? "text-purple-600" : "text-slate-400"}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
      ...
      <div className="relative z-10 mt-8">
        <Link
          href={`/become-vendor?plan=${name.toLowerCase()}`}
          className="block w-full"
        >
          <span
            className={`block w-full py-4 rounded-xl text-center text-sm font-bold transition-all cursor-pointer ${
              popular
    ...
    <section className="py-24 sm:py-32 bg-white relative overflow-hidden text-slate-900 border-t border-slate-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          >
            Simple, predictable monthly pricing designed to fit local clothing
            stores at any stage of growth.
          </motion.p>
        </div>
