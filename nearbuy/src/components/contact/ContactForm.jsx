...
const onboardingSchema = z.object({
  ...
  message: z
    .string()
    .min(10, "Introduction details must be at least 10 characters"),
});
...
export default function ContactForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Format payload to match your backend expectations
    const payload =
      formType === "enquiry"
        ...
        : {
            name,
            email,
            subject: `Store Onboarding: ${storeName} (${storeCategory})`,
    ...
    try {
      ...
      toast.success(
        formType === "enquiry"
          ...
          : "Onboarding request submitted! Our team will contact you within 12-24 hours.",
        { id: toastId },
      );
  ...
  return (
    ...
    >
      {/* Spotlight cursor inner light-orb glow (Updated to Blue) */}
      <div
        className="absolute inset-0 pointer-events-none transition-opacity duration-300 z-10"
        style={{
          opacity: isHovering ? 1 : 0,
          background: `radial-gradient(130px circle at ${hoverPos.x}px ${hoverPos.y}px, rgba(37, 99, 235, 0.05), transparent 80%)`,
        }}
      />
      ...
      <div
        ...
        style={{
          borderColor: isHovering ? "rgba(37, 99, 235, 0.2)" : "transparent",
        }}
      />
          ...
          <div className="space-y-4">
            <div className="flex bg-slate-100/80 backdrop-blur-xs p-1 rounded-2xl text-[10px] font-bold border border-slate-200/40 w-fit">
              <button
                ...
                className={`px-3 py-2 rounded-xl cursor-pointer transition-all duration-200 font-heading tracking-tight ${
                  formType === "enquiry"
            ...
            <h3 className="font-heading font-black text-slate-900 text-2xl tracking-tight leading-tight">
              {formType === "onboarding"
                ? "List Your Store On Streetunics"
                : "General Questions & Assistance"}
            </h3>
            <p className="text-slate-500 text-xs sm:text-sm leading-relaxed font-semibold">
              {formType === "onboarding"
                ? "Join the fastest growing hyperlocal clothing discovery network. Connect physical racks with digital search intent."
                : "Need support, API keys, custom listings, or corporate alliances? Reach out and we'll reply shortly."}
            </p>
          </div>

          {/* Quick specs lists */}
          <div className="space-y-4.5 bg-slate-50/50 border border-slate-100 p-5 rounded-2xl">
            <h4 className="font-heading font-black text-[11px] uppercase tracking-wider text-slate-400">
              Onboarding Highlights
            </h4>
            <div className="space-y-3">
              <div className="flex items-center gap-2.5 text-xs text-slate-700">
                <div className="h-5 w-5 bg-blue-50 text-blue-600 rounded-md flex items-center justify-center border border-blue-100/40 shrink-0">
                  ✓
                </div>
              ...
              <div className="flex items-center gap-2.5 text-xs text-slate-700">
                ...
                <span className="font-semibold">
                  Fast 12-24h verification review
                </span>
              </div>
              <div className="flex items-center gap-2.5 text-xs text-slate-700">
                <div className="h-5 w-5 bg-blue-50 text-blue-600 rounded-md flex items-center justify-center border border-blue-100/40 shrink-0">
                ...
                <span className="font-semibold">
                  Unlimited catalog discovery views
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-8 border-t lg:border-t-0 lg:border-l border-slate-100 pt-6 lg:pt-0 lg:pl-8">
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              ...
              {formType === "onboarding" ? (
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col space-y-1.5">
                      ...
                      <select
                        ...
                        className={`w-full text-sm block border rounded-lg pl-3 pr-8 py-2.5 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-white/50 backdrop-blur-xs transition-all font-semibold text-slate-800
                  ...
                  <Input
                    ...
                    name="storeLocation"
                    placeholder="E.g., Main Crossing, City Center"
                    required
                    value={storeLocation}
                    onChange={(e) =>
                      handleInputChange(
                ...
                >
                  <Input
                    ...
                    value={subject}
                    onChange={(e) =>
                      handleInputChange("subject", e.target.value, setSubject)
                    }
            ...
            <div className="flex flex-col space-y-1.5">
              <label className="text-xs font-semibold text-slate-700">
                {formType === "onboarding"
                  ? "Introduce Your Store / Brands Carried *"
                  : "Message / Comments *"}
              </label>
              <textarea
                rows={4}
                placeholder={
                  formType === "onboarding"
                    ? "Describe your dress styles, brand offerings, or registration questions..."
                    : "Explain your queries or support request in detail..."
                }
                value={message}
                onChange={(e) =>
                  handleInputChange("message", e.target.value, setMessage)
                }
                className={`w-full text-sm block border rounded-lg pl-3 pr-3 py-2.5 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-white/50 backdrop-blur-xs transition-all placeholder:text-slate-400 font-semibold text-slate-800
            ...
            <div className="pt-2">
              <motion.button
                ...
                disabled={isLoading}
                whileTap={{ scale: 0.98 }}
                className="w-full sm:w-auto px-6 py-3 rounded-xl text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-md hover:shadow-lg shadow-blue-600/20 cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2 select-none transition-all"
              >
                {isLoading ? (
                  <>
                    <svg
